    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if currentPosition in self.corners:
                ...
                visited_corners.add(currentPosition)
            ...
            if not hitsWall:
                ...
                nextState = (nextPosition, visited_corners)
                if nextPosition in self.corners:
                    visited_corners = set(visited_corners)
                    visited_corners.add(nextPosition)
                    visited_corners = tuple(visited_corners)
                    nextState = (nextPosition, visited_corners)
                cost = self.costFn(nextState)
                successors.append((nextState, action, cost))
...
def cornersHeuristic(state, problem):
    ...
    if len(visited_corners) < len(corners):
        cost = max([manhattan(coord, position) for coord in corners
                    if coord not in visited_corners])
    "*** YOUR CODE HERE ***"
...
def foodHeuristic(state, problem):
    ...
    def count_walls_between(fx, fy):
        return sum([wx in range(min(x, fx), max(x, fx)+1) and
                    wy in range(min(y, fy), max(y, fy)+1) for
                   (wx, wy) in wallList])
    ...
    if len(food_positions):
        # max_walls = max([count_walls_between(fx, fy) for fx, fy in food_positions])
        # max_euc = max([euclidean(position, f) for f in food_positions])
        # max_man = max([manhattan(position, f) for f in food_positions])
